...
- **Bug**: Hours before the submission deadline, the confirmation email was not being sent, prompting me to contact tutor support for assistance. The issue was traced to an error in the webhooks.py. After fixing this error, the webhook handler started giving a 500 error even though the payment intent succeeded. I spent several hours trying to identify the cause of this error. Unfortunately, tutor support time ended at that crucial moment, which felt unfair as I hadn't utilized tutor support since June or July. I dedicated the last 8 hours to resolving this issue, but regrettably, I couldn't find a solution before the submission deadline. 500 errors are notoriously challenging to troubleshoot, and I hope that the extensive functionality and hard work invested in the rest of the project can compensate for this issue. 
